...
- **Global scope**: `/root/headscale-management/.cursor/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/root/headscale-management/.cursor/notes/`)
...
2. Glob `/root/headscale-management/.cursor/notes/*.md` (if directory exists) — global notes
...
Global (/root/headscale-management/.cursor/notes/):
...
2. **No `.planning/`**: Falls back to global `/root/headscale-management/.cursor/notes/` — works in any directory
