...
<required_reading>
@/root/headscale-management/.cursor/gsd-core/references/domain-probes.md
@/root/headscale-management/.cursor/gsd-core/references/gate-prompts.md
@/root/headscale-management/.cursor/gsd-core/references/universal-anti-patterns.md
</required_reading>
...
if [ -f "/root/headscale-management/.cursor/gsd-core/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
Read `@/root/headscale-management/.cursor/gsd-core/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
